...
-------------------------------------------------------------------------------------------------
function OnPopupMessage(player, city, isFoundingReligion)
	g_city = city;
	g_player = player;
	g_bFoundingReligion = isFoundingReligion;
   	UIManager:QueuePopup( ContextPtr, PopupPriority.SocialPolicy );
end
LuaEvents.IGE_ChooseReligionPopup.Add(OnPopupMessage);
...
function RefreshReligions()
	...
	for row in GameInfo.Religions("Type <> 'RELIGION_PANTHEON'") do
		...
		if (g_bFoundingReligion) then
			religionButton.Button:RegisterCallback(Mouse.eLClick, function() SelectReligion(row.ID, row.Description, row.IconAtlas, row.PortraitIndex); end);
		else
