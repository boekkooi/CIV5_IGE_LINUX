...
GetYieldTooltip = GetYieldTooltip or function(pCity, iYieldType, iBase, iTotal, strIconString, strModifiersString)
	...
	if (iYieldFromTerrain ~= 0) then
		strYieldBreakdown = strYieldBreakdown .. "[ICON_BULLET]" .. IGEL("TXT_KEY_YIELD_FROM_TERRAIN", iYieldFromTerrain, strIconString);
	...
	if (iYieldFromBuildings ~= 0) then
		strYieldBreakdown = strYieldBreakdown .. "[ICON_BULLET]" .. IGEL("TXT_KEY_YIELD_FROM_BUILDINGS", iYieldFromBuildings, strIconString);
		strYieldBreakdown = strYieldBreakdown .. "[NEWLINE]";
	end
	...
	if (iYieldFromSpecialists ~= 0) then
		strYieldBreakdown = strYieldBreakdown .. "[ICON_BULLET]" .. IGEL("TXT_KEY_YIELD_FROM_SPECIALISTS", iYieldFromSpecialists, strIconString);
		strYieldBreakdown = strYieldBreakdown .. "[NEWLINE]";
	end
	...
	if (iYieldFromMisc ~= 0) then
		if (iYieldType == YieldTypes.YIELD_SCIENCE) then
			strYieldBreakdown = strYieldBreakdown .. "[ICON_BULLET]" .. IGEL("TXT_KEY_YIELD_FROM_POP", iYieldFromMisc, strIconString);
		else
			strYieldBreakdown = strYieldBreakdown .. "[ICON_BULLET]" .. IGEL("TXT_KEY_YIELD_FROM_MISC", iYieldFromMisc, strIconString);
		end
		strYieldBreakdown = strYieldBreakdown .. "[NEWLINE]";
	end
	...
	if (iYieldPerPop ~= 0) then
		...
		strYieldBreakdown = strYieldBreakdown .. "[ICON_BULLET]" .. IGEL("TXT_KEY_YIELD_FROM_POP_EXTRA", iYieldFromPop, strIconString);
		strYieldBreakdown = strYieldBreakdown .. "[NEWLINE]";
	end
	...
	if (iYieldType == YieldTypes.YIELD_FOOD) then
		...
		if (iYieldEaten ~= 0) then
			...
			--strModifiers = strModifiers .. "[ICON_BULLET]" .. IGEL("TXT_KEY_YIELD_EATEN_BY_POP", iYieldEaten, "[ICON_FOOD]");
			--strModifiers = strModifiers .. "[NEWLINE]----------------[NEWLINE]";
			
			strExtraBaseString = strExtraBaseString .. "   " .. IGEL("TXT_KEY_FOOD_USAGE", iBase, iYieldEaten);
			
			local iFoodSurplus = pCity:GetYieldRate(YieldTypes.YIELD_FOOD) - iYieldEaten;
			iBase = iFoodSurplus;
			
			--if (iFoodSurplus >= 0) then
				--strModifiers = strModifiers .. IGEL("TXT_KEY_YIELD_AFTER_EATEN", iFoodSurplus, "[ICON_FOOD]");
			--else
				--strModifiers = strModifiers .. IGEL("TXT_KEY_YIELD_AFTER_EATEN_NEGATIVE", iFoodSurplus, "[ICON_FOOD]");
			--end
		end
	end
	...
	local strTotal;
	if (iTotal >= 0) then
		strTotal = IGEL("TXT_KEY_YIELD_TOTAL", iTotal, strIconString);
	else
		strTotal = IGEL("TXT_KEY_YIELD_TOTAL_NEGATIVE", iTotal, strIconString);
	end
	...
	if (iBase ~= iTotal or strExtraBaseString ~= "") then
		local strBase = IGEL("TXT_KEY_YIELD_BASE", iBase, strIconString) .. strExtraBaseString;
...
GetHelpTextForUnit = GetHelpTextForUnit or function(iUnitID, bIncludeRequirementsInfo)
	...
	-- Name
	strHelpText = strHelpText .. Locale.ToUpper(IGEL( pUnitInfo.Description ));
	
	-- Cost
	strHelpText = strHelpText .. "[NEWLINE]----------------[NEWLINE]";
	strHelpText = strHelpText .. IGEL("TXT_KEY_PRODUCTION_COST", pActivePlayer:GetUnitProductionNeeded(iUnitID));
	-- Moves
	strHelpText = strHelpText .. "[NEWLINE]";
	strHelpText = strHelpText .. IGEL("TXT_KEY_PRODUCTION_MOVEMENT", pUnitInfo.Moves);
	
	-- Range
	local iRange = pUnitInfo.Range;
	if (iRange ~= 0) then
		strHelpText = strHelpText .. "[NEWLINE]";
		strHelpText = strHelpText .. IGEL("TXT_KEY_PRODUCTION_RANGE", iRange);
	end
	...
	if (iRangedStrength ~= 0) then
		strHelpText = strHelpText .. "[NEWLINE]";
		strHelpText = strHelpText .. IGEL("TXT_KEY_PRODUCTION_RANGED_STRENGTH", iRangedStrength);
	end
	...
	if (iStrength ~= 0) then
		strHelpText = strHelpText .. "[NEWLINE]";
		strHelpText = strHelpText .. IGEL("TXT_KEY_PRODUCTION_STRENGTH", iStrength);
	end
	...
	for pResource in GameInfo.Resources() do
		...
		if (iNumResourceNeeded > 0) then
			...
			if (iNumResourcesNeededSoFar == 0) then
				strHelpText = strHelpText .. "[NEWLINE]";
				strHelpText = strHelpText .. IGEL("TXT_KEY_PRODUCTION_RESOURCES_REQUIRED");
				strHelpText = strHelpText .. " " .. iNumResourceNeeded .. " " .. pResource.IconString .. " " .. IGEL(pResource.Description);
			else
				strHelpText = strHelpText .. ", " .. iNumResourceNeeded .. " " .. pResource.IconString .. " " .. IGEL(pResource.Description);
			end
	...
	if (not pUnitInfo.Help) then
		...
		local strWrittenHelpText = IGEL( pUnitInfo.Help );
	...
	if (bIncludeRequirementsInfo) then
		if (pUnitInfo.Requirements) then
			strHelpText = strHelpText .. IGEL( pUnitInfo.Requirements );
		end
	end
...
GetHelpTextForBuilding = GetHelpTextForBuilding or function(iBuildingID, bExcludeName, bExcludeHeader, bNoMaintenance)
	...
	if (not bExcludeHeader) then
		...
		if (not bExcludeName) then
			-- Name
			strHelpText = strHelpText .. Locale.ToUpper(IGEL( pBuildingInfo.Description ));
			strHelpText = strHelpText .. "[NEWLINE]----------------[NEWLINE]";
		end
		
		-- Cost
		local iCost = pActivePlayer:GetBuildingProductionNeeded(iBuildingID);
		strHelpText = strHelpText .. IGEL("TXT_KEY_PRODUCTION_COST", iCost);
		...
		if (not bNoMaintenance) then
			...
			if (iMaintenance ~= nil and iMaintenance ~= 0) then
				strHelpText = strHelpText .. "[NEWLINE]";
				strHelpText = strHelpText .. IGEL("TXT_KEY_PRODUCTION_BUILDING_MAINTENANCE", iMaintenance);
			end
		end
	...
	if (iHappinessTotal ~= 0) then
		strHelpText = strHelpText .. "[NEWLINE]";
		strHelpText = strHelpText .. IGEL("TXT_KEY_PRODUCTION_BUILDING_HAPPINESS", iHappinessTotal);
	end
	...
	if (iCulture ~= nil and iCulture ~= 0) then
		strHelpText = strHelpText .. "[NEWLINE]";
		strHelpText = strHelpText .. IGEL("TXT_KEY_PRODUCTION_BUILDING_CULTURE", iCulture);
	end
	...
	if (iDefense ~= nil and iDefense ~= 0) then
		strHelpText = strHelpText .. "[NEWLINE]";
		strHelpText = strHelpText .. IGEL("TXT_KEY_PRODUCTION_BUILDING_DEFENSE", iDefense / 100);
	end
	...
	if (iFood ~= nil and iFood ~= 0) then
		strHelpText = strHelpText .. "[NEWLINE]";
		strHelpText = strHelpText .. IGEL("TXT_KEY_PRODUCTION_BUILDING_FOOD", iFood);
	end
	...
	if (iGold ~= nil and iGold ~= 0) then
		strHelpText = strHelpText .. "[NEWLINE]";
		strHelpText = strHelpText .. IGEL("TXT_KEY_PRODUCTION_BUILDING_GOLD", iGold);
	end
	...
	if (iGold ~= nil and iGold ~= 0) then
		strHelpText = strHelpText .. "[NEWLINE]";
		strHelpText = strHelpText .. IGEL("TXT_KEY_PRODUCTION_BUILDING_GOLD_CHANGE", iGold);
	end
	...
	if (iScience ~= nil and iScience ~= 0) then
		strHelpText = strHelpText .. "[NEWLINE]";
		strHelpText = strHelpText .. IGEL("TXT_KEY_PRODUCTION_BUILDING_SCIENCE", iScience);
	end
	...
	if (iScienceChange ~= nil and iScienceChange ~= 0) then
		strHelpText = strHelpText .. "[NEWLINE]";
		strHelpText = strHelpText .. IGEL("TXT_KEY_PRODUCTION_BUILDING_SCIENCE_CHANGE", iScienceChange);
	end
	...
	if (iProduction ~= nil and iProduction ~= 0) then
		strHelpText = strHelpText .. "[NEWLINE]";
		strHelpText = strHelpText .. IGEL("TXT_KEY_PRODUCTION_BUILDING_PRODUCTION", iProduction);
	end
	...
	if (iProd ~= nil and iProd ~= 0) then
		strHelpText = strHelpText .. "[NEWLINE]";
		strHelpText = strHelpText .. IGEL("TXT_KEY_PRODUCTION_BUILDING_PRODUCTION_CHANGE", iProd);
	end
	...
	if IGE_HasGodsAndKings then
		...
		if (iFaith ~= nil and iFaith ~= 0) then
			strHelpText = strHelpText .. "[NEWLINE]";
			strHelpText = strHelpText .. IGEL("TXT_KEY_PRODUCTION_BUILDING_FAITH", iCulture);
		end
	end
	...
	if specialistType ~= nil then
		...
		if (iNumPoints > 0) then
			strHelpText = strHelpText .. "[NEWLINE]";
			strHelpText = strHelpText .. "[ICON_GREAT_PEOPLE] " .. IGEL(GameInfo.Specialists[specialistType].GreatPeopleTitle) .. " " .. iNumPoints;
		end
		...
		if(pBuildingInfo.SpecialistCount > 0) then
			...
			strHelpText = strHelpText .. "[ICON_GREAT_PEOPLE] " .. IGEL(specialistSlotsKey) .. " " .. pBuildingInfo.SpecialistCount;
		end
	...
	if (pBuildingInfo.Help ~= nil) then
		local strWrittenHelpText = IGEL( pBuildingInfo.Help );
		if (strWrittenHelpText ~= nil and strWrittenHelpText ~= "") then
			-- Separator
			strHelpText = strHelpText .. "[NEWLINE]----------------[NEWLINE]";
			strHelpText = strHelpText .. strWrittenHelpText;
...
GetFoodTooltip = GetFoodTooltip or function(pCity)
	...
	if (not OptionsManager.IsNoBasicHelp()) then
		strFoodToolTip = strFoodToolTip .. IGEL("TXT_KEY_FOOD_HELP_INFO");
	...
	strFoodToolTip = strFoodToolTip .. IGEL("TXT_KEY_FOOD_PROGRESS", fFoodProgress, iFoodNeeded);
...
GetGoldTooltip = GetGoldTooltip or function(pCity)
	...
	if (not OptionsManager.IsNoBasicHelp()) then
		strGoldToolTip = strGoldToolTip .. IGEL("TXT_KEY_GOLD_HELP_INFO");
...
GetScienceTooltip = GetScienceTooltip or function(pCity)
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_SCIENCE)) then
		strScienceToolTip = IGEL("TXT_KEY_TOP_PANEL_SCIENCE_OFF_TOOLTIP");
	else

		local iYieldType = YieldTypes.YIELD_SCIENCE;
	
		if (not OptionsManager.IsNoBasicHelp()) then
			strScienceToolTip = strScienceToolTip .. IGEL("TXT_KEY_SCIENCE_HELP_INFO");
...
GetProductionTooltip = GetProductionTooltip or function(pCity)
	...
	if (not OptionsManager.IsNoBasicHelp()) then
		strProductionHelp = strProductionHelp .. IGEL("TXT_KEY_PRODUCTION_HELP_INFO");
		strProductionHelp = strProductionHelp .. "[NEWLINE][NEWLINE]";
		--Controls.ProductionButton:SetToolTipString(IGEL("TXT_KEY_CITYVIEW_CHANGE_PROD_TT"));
	else
		--Controls.ProductionButton:SetToolTipString(IGEL("TXT_KEY_CITYVIEW_CHANGE_PROD"));
	end
...
GetCultureTooltip = GetCultureTooltip or function(pCity)
	...
	if (not OptionsManager.IsNoBasicHelp()) then
		strCultureToolTip = strCultureToolTip .. IGEL("TXT_KEY_CULTURE_HELP_INFO");
	...
	if (iCultureFromBuildings ~= 0) then
		...
		strCultureToolTip = strCultureToolTip .. "[ICON_BULLET]" .. IGEL("TXT_KEY_CULTURE_FROM_BUILDINGS", iCultureFromBuildings);
	end
	...
	if (iCultureFromPolicies ~= 0) then
		...
		strCultureToolTip = strCultureToolTip .. "[ICON_BULLET]" .. IGEL("TXT_KEY_CULTURE_FROM_POLICIES", iCultureFromPolicies);
	end
	...
	if (iCultureFromSpecialists ~= 0) then
		...
		strCultureToolTip = strCultureToolTip .. "[ICON_BULLET]" .. IGEL("TXT_KEY_CULTURE_FROM_SPECIALISTS", iCultureFromSpecialists);
	end
	...
	if (iCultureFromTerrain ~= 0) then
		...
		strCultureToolTip = strCultureToolTip .. "[ICON_BULLET]" .. IGEL("TXT_KEY_CULTURE_FROM_TERRAIN", iCultureFromTerrain);
	end
	...
	if (iCultureFromTraits ~= 0) then
		...
		strCultureToolTip = strCultureToolTip .. "[ICON_BULLET]" .. IGEL("TXT_KEY_CULTURE_FROM_TRAITS", iCultureFromTraits);
	end
	...
	if (iAmount ~= 0) then
		strCultureToolTip = strCultureToolTip .. "[NEWLINE][NEWLINE]";
		strCultureToolTip = strCultureToolTip .. "[ICON_BULLET]" .. IGEL("TXT_KEY_CULTURE_PLAYER_MOD", iAmount);
	end
	...
	if (iAmount ~= 0) then
		strCultureToolTip = strCultureToolTip .. "[NEWLINE][NEWLINE]";
		strCultureToolTip = strCultureToolTip .. "[ICON_BULLET]" .. IGEL("TXT_KEY_CULTURE_CITY_MOD", iAmount);
	end
	...
	if (pCity:GetNumWorldWonders() > 0) then
		...
		if (iAmount ~= 0) then
			strCultureToolTip = strCultureToolTip .. "[NEWLINE][NEWLINE]";
			strCultureToolTip = strCultureToolTip .. "[ICON_BULLET]" .. IGEL("TXT_KEY_CULTURE_WONDER_BONUS", iAmount);
		end
	end
	...
	strCultureToolTip = strCultureToolTip .. "[NEWLINE][NEWLINE]";
	strCultureToolTip = strCultureToolTip .. IGEL("TXT_KEY_CULTURE_INFO", iCultureStored, iCultureNeeded);
	...
	if iCulturePerTurn > 0 then
		...
		strCultureToolTip = strCultureToolTip .. " " .. IGEL("TXT_KEY_CULTURE_TURNS", iCultureTurns);
	end
...
GetIGEHelpTextForUnit = function(row, activePlayer)
	local help = "";
	help = help..IGEL("TXT_KEY_PRODUCTION_COST", activePlayer:GetUnitProductionNeeded(row.ID));
	help = help.."[NEWLINE]"..IGEL("TXT_KEY_PRODUCTION_MOVEMENT", row.Moves);
	if (row.Range ~= 0) then
		help = help.."[NEWLINE]"..IGEL("TXT_KEY_PRODUCTION_RANGE", row.Range);
	end
	if (row.RangedCombat ~= 0) then
		help = help.."[NEWLINE]"..IGEL("TXT_KEY_PRODUCTION_RANGED_STRENGTH", row.RangedCombat);
	end
	if (row.Combat ~= 0) then
		help = help.."[NEWLINE]"..IGEL("TXT_KEY_PRODUCTION_STRENGTH", row.Combat);
	end
	if row.Help then
		local subHelp = IGEL(row.Help);
		if (strWrittenHelpText ~= nil and strWrittenHelpText ~= "") then
			help = help.."[NEWLINE]"..subHelp;
		end
	end
	if (row.Requirements) then
		help = help.."[NEWLINE]"..IGEL(row.Requirements);
	end
	return help
end
...
GetIGEHelpTextForTech = function(tech)
	...
	if #tech.prereqs > 0 then
		help = IGEL("TXT_KEY_IGE_TECH_PREREQ").." "..IGEimplode(tech.prereqs, "name", ", ");
	end
	...
	if #tech.buildings > 0 then
		...
		help = help.."[COLOR_CYAN][ICON_BULLET]"..IGEimplode(tech.buildings, "name", "[NEWLINE][ICON_BULLET]").."[ENDCOLOR]";
	end
	...
	if #tech.wonders > 0 then
		...
		help = help.."[COLOR_XP_BLUE][ICON_BULLET]"..IGEimplode(tech.wonders, "name", "[NEWLINE][ICON_BULLET]").."[ENDCOLOR]";
	end
	...
	if #tech.units > 0 then
		...
		help = help.."[COLOR_UNIT_TEXT][ICON_BULLET]"..IGEimplode(tech.units, "name", "[NEWLINE][ICON_BULLET]").."[ENDCOLOR]";
	end
