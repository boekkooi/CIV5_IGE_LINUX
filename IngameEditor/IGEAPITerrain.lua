...
-------------------------------------------------------------------------------------------------
function IGEInvalidateTerrain(plot, resourceChanged)
...
-------------------------------------------------------------------------------------------------
function IGECommitTerrainChanges()
	...
	for i, pPlot in ipairs(invalidatedPlots) do
		...
		for neighbor in IGENeighbors(pPlot) do
...
-------------------------------------------------------------------------------------------------
function IGECanHaveNaturalWonder(plot, wonder)
	-- Gibraltar
...
-------------------------------------------------------------------------------------------------
function IGECanHaveFeature(plot, feature)
...
-------------------------------------------------------------------------------------------------
function IGECanHaveResource(plot, resource)
...
-------------------------------------------------------------------------------------------------
function IGECanHaveImprovement(plot, improvement)
	...
	if plot:GetTerrainType() == TerrainTypes.TERRAIN_OCEAN then
		for neighbor in IGENeighbors(plot) do
			...
			if neighbor:GetPlotType() ~= PlotTypes.PLOT_OCEAN then 
				...
				IGEInvalidateTerrain(plot, true);
				break
			end
		end
	elseif plot:GetPlotType() ~= PlotTypes.PLOT_OCEAN then
		for neighbor in IGENeighbors(plot) do
			if neighbor:GetTerrainType() == TerrainTypes.TERRAIN_OCEAN then 
				...
				IGEInvalidateTerrain(neighbor, true);
			end
		end
	end
end
...
-------------------------------------------------------------------------------------------------
function IGESetTerrain(terrain, plot)
	if plot then
		-- Want to set coast ? 
...
-------------------------------------------------------------------------------------------------
function IGESetPlotType(type, plot)
...
-------------------------------------------------------------------------------------------------
function IGESetFoggy(plot)
	table.insert(plotsToFog, plot);
end

-------------------------------------------------------------------------------------------------
function IGESetFeature(feature, plot)
...
-------------------------------------------------------------------------------------------------
function IGESetResource(resource, plot)
...
-------------------------------------------------------------------------------------------------
function IGESetResourceQty(qty, plot)
...
-------------------------------------------------------------------------------------------------
function IGESetImprovement(improvement, plot)
...
-------------------------------------------------------------------------------------------------
function IGESetImprovementPillaged(value, plot)
...
-------------------------------------------------------------------------------------------------
function IGESetRoute(route, plot)
...
-------------------------------------------------------------------------------------------------
function IGESetContinentArt(art, plot)
...
function IGESetFog(data, plot)
	if plot then
		...
		Game.UpdateFOW(true);
		IGEInvalidateTerrain(plot);
		return true;
	end
end

-------------------------------------------------------------------------------------------------
function IGESetOwnership(data, plot)
	if data.value then
...
function IGEBackupPlot(plot)
	...
	for _, side in ipairs(RiverSides) do
		backup.rivers[side] = IGEGetFlowRotation(plot, side);
	end
...
-------------------------------------------------------------------------------------------------
function IGERestorePlot(backup)
	--print("restore plot");
	local team = IGE.currentPlayer:GetTeam();
	...
	for _, side in ipairs(RiverSides) do
		IGESetFlowRotation(plot, side, backup.rivers[side]);
	end
