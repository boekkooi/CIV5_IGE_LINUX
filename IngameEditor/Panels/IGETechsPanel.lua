...
local instances = {};
local eraInstances = {};
local eraManager = IGECreateInstanceManager("EraInstance", "Root", Controls.MainList );
local pipeManager = IGECreateInstanceManager("ConnectorInstance", "Root", Controls.ScrollPanel);
local techManager = IGECreateInstanceManager("TechInstance", "Button", Controls.MainList);
local fallbackEraManager = IGECreateInstanceManager("FallbackEraInstance", "Root", Controls.MainList );
...
table.insert(profiles, { name = IGEL("TXT_KEY_IGE_TECHS_COMPATIBILITY_NONE"), filter = function(t) return true end });
table.insert(profiles, { name = IGEL("TXT_KEY_IGE_TECHS_COMPATIBILITY_STANDARD"), filter = function(t) return ((t.gridX > 0) and (t.gridY > 0)) end });
table.insert(profiles, { name = IGEL("TXT_KEY_IGE_TECHS_COMPATIBILITY_NIGHTS"), filter = function(t) return not t.disable end });
...
function IGEInitializeControls()
	...
	for i, era in ipairs(data.eras) do
		if #era.techs > 0 then
			...
			if instance then
				instance.Button:RegisterCallback(Mouse.eLClick, function() IGEEraClickHandler(era) end);
				...
				if data.canUseVanillaLogicForTechs then
					...
					fallbackTechManagers[i] = IGECreateInstanceManager("FallbackTechInstance", "Button", instance.List );
	...
	-- Technologies
	if data.canUseVanillaLogicForTechs then
		local xmin, xmax = IGEUpdateGraph(data.techs, techManager, pipeManager, instances, sizeX, 38, paddingX, 38, offsetX - deltaEra, 27, true);
		...
		for i, tech in ipairs(data.techs) do
			if tech.visible then
				...
				instance.Button:SetToolTipCallback(function() IGEToolTipHandler(tech) end);
				instance.Button:RegisterCallback(Mouse.eLClick, function() IGEClickHandler(tech) end);
			end
		end
	end
...
function IGEOnInitialize()
	...
	IGECheckLayoutForTechs(data);
	IGESetBuildingsData(data);
	IGESetUnitsData(data);
	IGEInitializeHelp();

	IGEResize(Controls.MainStack);
	IGEResize(Controls.ScrollPanel);
	IGEResize(Controls.Container);

	IGEInitializeControls();

	local tt = IGEL("TXT_KEY_IGE_TECHS_PANEL_HELP");
	LuaEvents.IGE_RegisterTab("TECHS", IGEL("TXT_KEY_IGE_TECHS_PANEL"), 4, "change",  tt);
end
...
-------------------------------------------------------------------------------------------------
function IGEEraClickHandler(era)
	local pTeam = Teams[IGE.currentPlayer:GetTeam()];
	...
	for i, era in ipairs(data.eras) do
		...
		if instance then
			if data.canUseVanillaLogicForTechs then
				...
				local width = IGEUpdateList(era.techs, fallbackTechManagers[i], IGEClickHandler, nil, instances[era]);
...
Controls.CompatibilityPullDown:RegisterSelectionCallback(IGEOnPullDownSelectionChanged);
