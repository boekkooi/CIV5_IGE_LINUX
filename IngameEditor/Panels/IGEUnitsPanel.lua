...
local eraItemManager = IGECreateInstanceManager("GroupInstance", "Stack", Controls.EraList );
...
function IGEOnInitialize()
	...
	if civilianGroupInstance then
		civilianUnitManager = IGECreateInstanceManager("ListItemInstance", "Button", civilianGroupInstance.List );
		civilianGroupInstance.Header:SetText(IGEL("TXT_KEY_IGE_CIVILIAN_UNITS"));
	end

	if IGE_HasGodsAndKings then
		religiousGroupInstance = eraItemManager:GetInstance();
		if religiousGroupInstance then
			religiousUnitManager = IGECreateInstanceManager("ListItemInstance", "Button", religiousGroupInstance.List );
			religiousGroupInstance.Header:SetText(IGEL("TXT_KEY_IGE_RELIGIOUS_UNITS"));
			religiousGroupInstance.HeaderBackground:SetToolTipString(IGEL("TXT_KEY_IGE_RELIGIOUS_UNITS_HELP"));
		end
	end
	...
	for i, v in ipairs(data.eras) do
		if #v.units > 0 then
			...
			if instance then
				local manager = IGECreateInstanceManager("ListItemInstance", "Button", instance.List );
				...
				if not unit.isGreatPeople then
					for _, religion in pairs(data.religions) do
						if religion.ID > 0 then
							local c = IGEclone(unit);
							c.religion = religion.ID;
							c.priority = religion.ID;
							c.subtitle = IGEL(Game.GetReligionName(religion.ID));
							c.name = religion.iconString.." "..c.name.."*";
							c.note = IGEL("TXT_KEY_IGE_RELIGIOUS_UNITS_WARNING");
	...
	local tt = IGEL("TXT_KEY_IGE_UNITS_PANEL_HELP");
	LuaEvents.IGE_RegisterTab("UNITS", IGEL("TXT_KEY_IGE_UNITS_PANEL"), 2, "paint",  tt, currentUnit)
...
-------------------------------------------------------------------------------------------------
function IGETryPurchaseReligiousUnit(pPlayer, plot, start)
			...
			if success then
				...
				LuaEvents.IGE_Schedule(5, nil, function()
					...
					if pPlayer:GetFaith() ~= faith then
						...
						IGETryPurchaseReligiousUnit(pPlayer, plot, i + 1);
						return;
					end
...
-------------------------------------------------------------------------------------------------
function IGEOnPlop(mouseButtonDown, plot, shift)
...
-- UPDATE
--===============================================================================================
UpdateLevel = IGEHookNumericBox("Level", 
	function() return currentLevel end, 
	function(amount) currentLevel = amount end, 
	1, nil, 1);

-------------------------------------------------------------------------------------------------
function IGEUpdateUnitList(units, itemManager, instance)
	for _, unit in ipairs(units) do 
		unit.selected = (unit == currentUnit);
		unit.label = unit.name;
	end

	IGEUpdateHierarchizedList(units, itemManager, IGEClickHandler);
...
function IGEOnUpdate()
	...
	if IGE_HasGodsAndKings then
		...
		IGEUpdateUnitList(religiousUnits, religiousUnitManager, religiousGroupInstance);
	end

	-- Units
	IGEUpdateUnitList(civilianUnits, civilianUnitManager, civilianGroupInstance);
	for i, era in ipairs(data.eras) do
		if #era.units > 0 then
			IGEUpdateUnitList(era.units, unitItemManagers[i], groupInstances[i]);
		end
	end	
