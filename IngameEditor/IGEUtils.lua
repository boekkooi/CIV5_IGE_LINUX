...
-------------------------------------------------------------------------------------------------
function IGEimplode(items, keyName, separator)
...
-------------------------------------------------------------------------------------------------
function IGEgetFileName(path)
	local index = IGElastIndexOf(path, "\\") or IGElastIndexOf(path, "/");
	return string.sub(path, index + 1);
end

-------------------------------------------------------------------------------------------------
function IGEgetStackTrace()
	local trace = {};
	local level = 2
	while true do
		...
		if info.what == "C" then   -- is a C function?
			...
			-- a Lua function
			local userStr = IGEgetFileName(info.source)..": "..info.currentline;
...
-------------------------------------------------------------------------------------------------
function IGEFormatError(err, levelsToIgnoreBelow)
