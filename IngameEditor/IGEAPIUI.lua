...
-------------------------------------------------------------------------------------------------
function IGECreateInstanceManager(style, rootName, parentControl)
...
-- Tooltip
--===============================================================================================
function IGEToolTipHandler(item)
	if item then
		-- Icon
...
-- LISTS UPDATE
--===============================================================================================
function IGESetupInstance(v, instance, clickHandler, rightClickHandler)
	...
	if v.toolTip then
		...
		instance.Button:SetToolTipCallback(function() IGEToolTipHandler(v) end);
	...
	if instance.TopButton then
		if v.topData and v.selected then
			...
			instance.TopButton:SetToolTipCallback(function() IGEToolTipHandler(v.topData) end);
...
-------------------------------------------------------------------------------------------------
local function IGEResizeListItemInstance(instance, columnWidth, labelWidth)
...
-------------------------------------------------------------------------------------------------
function IGEUpdateGeneric(items, listManager, clickHandler, columnWidth)
	...
	for i, v in ipairs(items) do
		if v.visible then
			...
			if instance then
				IGESetupInstance(v, instance, clickHandler);
				if instance.NameLabel then
					IGEResizeListItemInstance(instance, columnWidth, labelWidth);
				end
			end
		end
	end
...
function IGEUpdateList(items, listManager, clickHandler, rightClickHandler, instances)
	...
	for i, v in ipairs(items) do
		if v.visible then
			...
			if instance then
				...
				IGESetupInstance(v, instance, clickHandler, rightClickHandler);
...
-------------------------------------------------------------------------------------------------
function IGEUpdateHierarchizedList(items, itemManager, clickHandler, rightClickHandler, instances)
	local currentCivID = IGE.currentPlayer:GetCivilizationType();
	local currentCivType = GameInfo.Civilizations[currentCivID].Type;
	table.sort(items, IGEDefaultSort);
	...
	-- Update list
	IGEUpdateList(items, itemManager, clickHandler, rightClickHandler, instances);
end
...
-------------------------------------------------------------------------------------------------
function IGEUpdateGraph(items, itemManager, pipeManager, instances, sizeX, sizeY, paddingX, paddingY, x0, y0, horizontal)
...
-- Numeric boxes
--===============================================================================================
function IGEHookNumericBox(prefix, getter, setter, min, max, step, controlsHolder)
...
function IGELowerSizeY(control, offset)
