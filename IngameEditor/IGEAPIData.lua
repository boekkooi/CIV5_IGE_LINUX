...
--------------------------------------------------------------
BUG_NoGraphicalUpdate = IGEL("TXT_KEY_IGE_NO_GRAPHICAL_UPDATE");
BUG_SavegameCorruption = IGEL("TXT_KEY_IGE_SAVE_GAME_CORRUPTION");
...
-------------------------------------------------------------------------------------------------
function IGEAppendIDAndTypeToHelp(item)
...
-------------------------------------------------------------------------------------------------
function IGESetPlayersData(data, options)
	...
	for i = 0, GameDefines.MAX_CIV_PLAYERS, 1 do
		...
		if row:IsEverAlive() then	-- Must check it to prevent a crash
			...
			item.civilizationName = IGEL(civInfo.ShortDescription);
			...
			if item.isCityState then
				...
				item.name = IGEL(minorCivInfo.Description);
	...
	if options.none then
		table.insert(players, 1, { ID = -1, name = IGEL("TXT_KEY_IGE_NONE"), priority=999, visible = true, enabled = true});
	end
end

-------------------------------------------------------------------------------------------------
function IGESetTerrainsData(data)
	if data.terrains then return end 
	data.terrainsByTypes = {};
	data.waterTerrains = {};
	data.terrains = {};

	local lakeName = IGEL("TXT_KEY_PLOTROLL_LAKE")
	local coastName = IGEL(GameInfo.Terrains["TERRAIN_COAST"].Description)

	for row in GameInfo.Terrains() do	
		local item = {};
		local name = IGEL(row.Description);
		...
		if item.water then
			...
			for row in GameInfo.Building_SeaPlotYieldChanges() do
				if IsValidBuilding(row.BuildingType) then
					...
					if building then
						table.insert(item.yieldChanges, { yieldType = row.YieldType, yield = row.Yield, name = IGEL(building.Description), type = "BUILDING", suffix = coastSuffix });
					else
		...
		if isCoast then
			for row in GameInfo.Building_LakePlotYieldChanges() do
				if IsValidBuilding(row.BuildingType) then
					local building = GameInfo.Buildings[row.BuildingType];
					if building then
						table.insert(item.yieldChanges, { yieldType = row.YieldType, yield = row.Yield, name = IGEL(building.Description), type = "BUILDING", suffix = lakeName });
					else
		...
		item.help = IGEGetYieldChangeString(item);
		IGEAppendIDAndTypeToHelp(item)
		...
		if isCoast then
			item.name = IGEL("TXT_KEY_IGE_COAST_OR_LAKE")
			item.help = IGEL("TXT_KEY_IGE_COAST_OR_LAKE_HELP").."[NEWLINE][NEWLINE]"..item.help
		...
		item.subtitle = IGEGetYieldString(item);
	...
	-- Sort
	table.sort(data.waterTerrains, IGEDefaultSort);
	table.sort(data.terrains, IGEDefaultSort);
end

-------------------------------------------------------------------------------------------------
--[[
function IGESetRiverData()
	row = GameInfo.FakeFeatures("Type = 'FEATURE_RIVER'")();
	local river = river;
	river.name = IGEL(row.Description);
	...
	for row in GameInfo.Building_RiverPlotYieldChanges() do
		if IsValidBuilding(row.BuildingType) then
			local building = GameInfo.Buildings[row.BuildingType];
			table.insert(river.yieldChanges, { yieldType = row.YieldType, yield = row.Yield, name = IGEL(building.Description), type = "BUILDING" });
		end
	end
	river.help = IGEGetYieldChangeString(river);
end]]
...
--===============================================================================================
function IGESetContinentArtsData(data)
	if data.continentArts then return end

	data.continentArts = {
		{ name = IGEL("TXT_KEY_IGE_AFRICA"), ID = 3, action = IGESetContinentArt, help = IGEL("TXT_KEY_IGE_AFRICA_HELP"), note = BUG_NoGraphicalUpdate, visible = true, enabled = true },
		{ name = IGEL("TXT_KEY_IGE_AMERICA"), ID = 1, action = IGESetContinentArt, help = IGEL("TXT_KEY_IGE_AMERICA_HELP"), note = BUG_NoGraphicalUpdate, visible = true, enabled = true },
		{ name = IGEL("TXT_KEY_IGE_ASIA"), ID = 2, action = IGESetContinentArt, help = IGEL("TXT_KEY_IGE_ASIA_HELP"), note = BUG_NoGraphicalUpdate, visible = true, enabled = true },
		{ name = IGEL("TXT_KEY_IGE_EUROPE"), ID = 4, action = IGESetContinentArt, help = IGEL("TXT_KEY_IGE_EUROPE_HELP") , note = BUG_NoGraphicalUpdate, visible = true, enabled = true } };
end

-------------------------------------------------------------------------------------------------
function IGESetPlotTypesData(data)
	...
	data.types[0] = { ID = 0, type = PlotTypes.PLOT_LAND, name = IGEL("TXT_KEY_IGE_FLAT_LAND"), texture = tex, textureOffset = Vector2(0, 0) }
	...
	for i, v in ipairs(data.types) do
		v.action = IGESetPlotType;
		v.visible = true;
		v.enabled = true;
		v.note = BUG_NoGraphicalUpdate;
	end

	hills.subtitle = IGEGetYieldString(hills);
end

-------------------------------------------------------------------------------------------------
function IGESetFeaturesData(data, options)
	...
	for row in GameInfo.Features() do
		...
		for row in GameInfo.Building_FeatureYieldChanges(item.condition) do
			if IsValidBuilding(row.BuildingType) then
				local building = GameInfo.Buildings[row.BuildingType];
				if building then
					table.insert(item.yieldChanges, { yieldType = row.YieldType, yield = row.Yield, name = IGEL(building.Description), type = "BUILDING" });
				else
	...
	-- Sort
	table.sort(data.features, IGEDefaultSort);
	table.sort(data.naturalWonders, IGEDefaultSort);

	if options.none then
		table.insert(data.features, 1, { ID = -1, type = -1, name = IGEL("TXT_KEY_IGE_NONE"), visible = true, enabled = true, action = SetFeature });
	end
end

-------------------------------------------------------------------------------------------------
function IGESetResourcesData(data, options)
	...
	data.bonusResources = {};
 	...
 	for row in GameInfo.Resources() do
		...
		for row in GameInfo.Improvement_ResourceType_Yields(item.condition) do
			...
			if improvement then
				table.insert(item.yieldChanges, { yieldType = row.YieldType, yield = row.Yield, name = IGEL(improvement.Description), type = "IMPROVEMENT" });
		...
		if not improvementBringsBonus then
			...
			if improvementData then
				...
				if improvement then
					table.insert(item.yieldChanges, { yield = 0, name = IGEL(improvement.Description), type = "IMPROVEMENT" });
				else
		...
		for row in GameInfo.Building_ResourceCultureChanges(item.condition) do
			if IsValidBuilding(row.BuildingType) then
				local building = GameInfo.Buildings[row.BuildingType];
				if building then
					table.insert(item.yieldChanges, { yieldType = "YIELD_CULTURE", yield = row.CultureChange, name = IGEL(building.Description), type = "BUILDING" });
				else
	...
	-- Sort
	table.sort(data.bonusResources, IGEDefaultSort);
	table.sort(data.luxuryResources, IGEDefaultSort);
	table.sort(data.strategicResources, IGEDefaultSort);

	if options.none then
		table.insert(data.bonusResources, 1, { ID = -1, type = -1, name = IGEL("TXT_KEY_IGE_NONE"), visible = true, enabled = true, note = BUG_NoGraphicalUpdate, action = SetResource });
...
-- Improvements & routes
--===============================================================================================
function IGESetImprovementsData(data, options)
	...
	for row in GameInfo.Improvements() do	
		...
		for row in GameInfo.Improvement_TechYieldChanges(item.condition) do
			...
			if tech then
				table.insert(item.yieldChanges, { yieldType = row.YieldType, yield = row.Yield, name = IGEL(tech.Description), type = "TECH" });
			else
		...
		for row in GameInfo.Improvement_TechNoFreshWaterYieldChanges(item.condition) do
			local tech = GameInfo.Technologies[row.TechType]
			if tech then
				table.insert(item.yieldChanges, { yieldType = row.YieldType, yield = row.Yield, name = IGEL(tech.Description).." (no fresh water)", type = "TECH" });
			else
		...
		for row in GameInfo.Improvement_TechFreshWaterYieldChanges(item.condition) do
			local tech = GameInfo.Technologies[row.TechType]
			if tech then
				table.insert(item.yieldChanges, { yieldType = row.YieldType, yield = row.Yield, name = IGEL(tech.Description).." (fresh water)", type = "TECH" });
			else
	...
	-- Sort
	table.sort(data.improvements, IGEDefaultSort);
	table.sort(data.greatImprovements, IGEDefaultSort);

	if options.none then
		table.insert(data.greatImprovements, 1, { ID = -1, type = -1, name = IGEL("TXT_KEY_IGE_NONE"), visible = true, enabled = true, action = SetImprovement });
	end
end

-------------------------------------------------------------------------------------------------
function IGESetRoutesData(data, options)
	...
	for row in GameInfo.Routes() do	
		...
		item.action = IGESetRoute;
		item.visible = true;
		item.enabled = true;
		item.yields = {};

		item.help = IGEL(row.Civilopedia or "").."[NEWLINE]"
		IGEAppendIDAndTypeToHelp(item)
	...
	if options.none then
		table.insert(data.routes, 1, { ID = -1, type = -1, name = IGEL("TXT_KEY_IGE_NONE"), visible = true, enabled = true, action = SetRoute });
	end
end
...
-------------------------------------------------------------------------------------------------
function IGESetTechnologiesData(data)
	...
	for _, era in ipairs(data.eras) do
		table.sort(era.techs, IGEDefaultSort);
	end
end

-------------------------------------------------------------------------------------------------
function IGECheckLayoutForTechs(data)
	local grid = { {}, {}, {}, {}, {}, {}, {}, {}, {}, {} };
	local errors = {};
...
-------------------------------------------------------------------------------------------------
--[[
function IGESetPromotionsData(data)
...
-------------------------------------------------------------------------------------------------
function IGESetUnitsData(data)
	local activePlayer = Players[Game.GetActivePlayer()];
	if data.unitsByTypes then return end
	IGESetTechnologiesData(data);
	data.unitsByTypes = {};
	data.unitsByID = {};
	SetErasData(data);
 	...
 	for row in GameInfo.Units() do
		...
		if overriding then
			...
			if civilization then
				item.civilizationName = IGEL(GameInfo.Civilizations[item.civilizationType].ShortDescription);
				item.subtitle = item.civilizationName;
			else
	...
	for _, era in pairs(data.erasByID) do
		table.sort(era.units, IGEDefaultSort);
	end
end

-------------------------------------------------------------------------------------------------
function IGESetBuildingsData(data)
	...
	data.wonders = {};
 	...
 	for row in GameInfo.Buildings() do
		...
		if item.isNationalWonder then
			item.priority = 1;
			item.subtitle = IGEL("TXT_KEY_IGE_NATIONAL_WONDER");
		elseif item.isWonder then
			item.subtitle = IGEL("TXT_KEY_IGE_WONDER");
		else
			item.subtitle = "";
		end
		...
		if item.type == "BUILDING_INTELLIGENCE_AGENCY" then
			item.note = IGEL("TXT_KEY_IGE_INTELLIGENCE_AGENCY_WARNING")
		end
	...
	-- Sort
	table.sort(data.wonders, IGEDefaultSort);
	table.sort(data.buildings, IGEDefaultSort);
end

-------------------------------------------------------------------------------------------------
function IGESetPoliciesData(data)
	...
	data.policyByTypes = {};
 	...
 	for row in GameInfo.Policies() do
		local item = {};
		local help = IGEL(row.Help or "").."[NEWLINE]";
		local name = IGEL(row.Description);
		item.ID = row.ID;
		item.name = name;
		item.help = help;
		...
		if branchType and branchType ~= "" then
			...
			if not branch then
				...
				if branchData then
					...
					branch.name = IGEL(branchData.Description);
					branch.help = IGEL(branchData.Help);
	...
	table.sort(data.policyBranches, IGEDefaultSort);
...
-------------------------------------------------------------------------------------------------
function IGECheckLayoutForPolicies(data)
	for _, branch in ipairs(data.policyBranches) do
		local grid = { {}, {}, {}, {}, {} };
		local errors = {};
...
-------------------------------------------------------------------------------------------------
function IGESetReligionsData(data)
