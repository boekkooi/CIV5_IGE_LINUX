...
-------------------------------------------------------------------------------------------------
function IGEDoResizeBanner(BannerInstance)
...
local function UpdateCityBanner(city)
	...
	elseif not Teams[iActiveTeam]:IsHasMet(owner:GetTeam()) then
		strToolTip = IGEL("TXT_KEY_HAVENT_MET");
	elseif Game.IsOption(GameOptionTypes.GAMEOPTION_ALWAYS_WAR) then
		strToolTip = IGEL("TXT_KEY_ALWAYS_AT_WAR_WITH_CITY");
	...
	if not instance then
	    ...
	    instance.BannerButton:RegisterCallback(Mouse.eLClick, IGEOnBannerClick);
	    instance.BannerButton:SetVoid1(city:GetX());
	    instance.BannerButton:SetVoid2(city:GetY());
	    
    	instance.CityBannerProductionButton:RegisterCallback( Mouse.eLClick, IGEOnProdClick );
	...
	local localizedCityName = IGEL(cityName);
	...
	if (not city:IsCapital() and owner:IsCapitalConnectedToCity(city) and not city:IsBlockaded()) then
		...
		instance.ConnectedIcon:SetToolTipString(IGEL("TXT_KEY_CITY_CONNECTED"));
	else
	...
	if city:IsBlockaded() then
		...
		instance.BlockadedIcon:SetToolTipString(IGEL("TXT_KEY_CITY_BLOCKADED"));
	else
	...
	if city:IsRazing() then
		...
		instance.RazingIcon:SetToolTipString(IGEL( "TXT_KEY_CITY_BURNING", tostring(city:GetRazingTurns()) ));
	else
	...
	if city:IsResistance() then
		...
		instance.ResistanceIcon:SetToolTipString(IGEL( "TXT_KEY_CITY_RESISTANCE", tostring(city:GetResistanceTurns()) ));
	else
	...
	if city:IsPuppet() then
		...
		instance.PuppetIcon:SetToolTipString(IGEL("TXT_KEY_CITY_PUPPET"));
	else
	...
	if city:IsOccupied() and not city:IsNoOccupiedUnhappiness() then
		...
		instance.OccupiedIcon:SetToolTipString(IGEL( "TXT_KEY_CITY_OCCUPIED"));
	else
	...
	if (city:IsFoodProduction() or city:FoodDifferenceTimes100() == 0) then
		cityGrowth = "-";
		instance.LeftBackground:SetToolTipString(IGEL("TXT_KEY_CITY_STOPPED_GROWING_TT", localizedCityName, cityPopulation));
	elseif city:FoodDifferenceTimes100() < 0 then
		cityGrowth = "[COLOR_WARNING_TEXT]-[ENDCOLOR]";
		instance.LeftBackground:SetToolTipString(IGEL("TXT_KEY_CITY_STARVING_TT",localizedCityName ));
	else
		instance.LeftBackground:SetToolTipString(IGEL("TXT_KEY_CITY_WILL_GROW_TT", localizedCityName, cityPopulation, cityPopulation+1, cityGrowth));
	end
	...
	convertedKey = IGEL(cityProductionName);
	instance.CityProductionName:SetText(convertedKey);
			
	if cityProductionName == "TXT_KEY_PRODUCTION_NO_PRODUCTION" then
		instance.RightBackground:SetToolTipString(IGEL( "TXT_KEY_CITY_NOT_PRODUCING", localizedCityName));
	else
		local productionTurnsLeft = city:GetProductionTurnsLeft();
		local tooltipString;
		if productionTurnsLeft > 99 then
			tooltipString = IGEL(IGEL("TXT_KEY_CITY_CURRENTLY_PRODUCING_99PLUS_TT", localizedCityName, cityProductionName));
		else
			tooltipString = IGEL(IGEL("TXT_KEY_CITY_CURRENTLY_PRODUCING_TT", localizedCityName, cityProductionName, productionTurnsLeft));
		end
...
-- EVENTS
--===============================================================================================
function IGEOnCityCreated(hexPos)
...
-------------------------------------------------------------------------------------------------
function IGEOnCitySetDamage(iPlayerID, iCityID, iDamage, iPreviousDamage)
...
-------------------------------------------------------------------------------------------------
function IGEOnSpecificCityInfoDirty(iPlayerID, iCityID, eUpdateType)
    ...
    while player ~= nil do
        if player:IsAlive() then
            for city in player:Cities() do
    			IGEOnCityCreated( ToHexFromGrid( Vector2( city:GetX(), city:GetY() ) ), player:GetID(), city:GetID() );
            end
        end
...
-- INPUTS
--===============================================================================================
function IGEOnBannerClick(x, y)
end

-------------------------------------------------------------------------------------------------
function IGEOnProdClick(playerID, cityID)
...
local function IGEOnShowing()
	...
	Events.SerialEventCityCreated.Add(IGEOnCityCreated);
	Events.SerialEventCityCaptured.Add(IGEOnCityDestroyed);
	Events.SerialEventCityInfoDirty.Add(IGEOnAllCityDirty);
	Events.SerialEventCityDestroyed.Add(IGEOnCityDestroyed);
	Events.SerialEventCitySetDamage.Add(IGEOnCitySetDamage);
	Events.SpecificCityInfoDirty.Add(IGEOnSpecificCityInfoDirty);
...
local function IGEOnClosing()
	Events.SerialEventCityCreated.Remove(IGEOnCityCreated);
	Events.SerialEventCityCaptured.Remove(IGEOnCityDestroyed);
	Events.SerialEventCityInfoDirty.Remove(IGEOnAllCityDirty);
	Events.SerialEventCityDestroyed.Remove(IGEOnCityDestroyed);
	Events.SerialEventCitySetDamage.Remove(IGEOnCitySetDamage);
	Events.SpecificCityInfoDirty.Remove(IGEOnSpecificCityInfoDirty);
end
