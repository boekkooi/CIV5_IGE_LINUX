...
-------------------------------------------------------------------------------------------------
function IGEOnPopupMessage(player, city, isFoundingReligion)
	g_city = city;
	g_player = player;
	g_bFoundingReligion = isFoundingReligion;
   	UIManager:QueuePopup( ContextPtr, PopupPriority.SocialPolicy );
end
LuaEvents.IGE_ChooseReligionPopup.Add(IGEOnPopupMessage);
...
-------------------------------------------------------------------------------------------------
function IGEIgnoreLeftClick( Id )
	-- just swallow it
end
...
-------------------------------------------------------------------------------------------------
function IGEOnPantheonBeliefClick()

	if(IGEToggleBeliefContext("PantheonBelief")) then
		...
		IGESelectFromBeliefs(availablePantheonBeliefs, function(belief)
...
-------------------------------------------------------------------------------------------------
function IGEOnFounderBeliefClick()

	if(IGEToggleBeliefContext("FounderBelief")) then
		...
		IGESelectFromBeliefs(availableFounderBeliefs, function(belief)
...
-------------------------------------------------------------------------------------------------
function IGEOnFollowerBeliefClick()
	if(IGEToggleBeliefContext("FollowerBelief")) then
		...
		IGESelectFromBeliefs(availableFollowerBeliefs, function(belief)
...
-------------------------------------------------------------------------------------------------
function IGEOnFollowerBelief2Click()

	if(IGEToggleBeliefContext("FollowerBelief2")) then
		local availableFollowerBeliefs = {};
		for i,v in ipairs(Game.GetAvailableFollowerBeliefs()) do
			local belief = GameInfo.Beliefs[v];
			if(belief ~= nil) then
...
-------------------------------------------------------------------------------------------------
function IGEOnEnhancerBeliefClick()

	if(IGEToggleBeliefContext("EnhancerBelief")) then
		...
		IGESelectFromBeliefs(availableEnhancerBeliefs, function(belief)
...
-------------------------------------------------------------------------------------------------
function IGEOnBonusBeliefClick()

	if(IGEToggleBeliefContext("BonusBelief")) then
		...
		IGESelectFromBeliefs(availableBonusBeliefs, function(belief)
...
-------------------------------------------------------------------------------------------------
function IGEToggleBeliefContext(contextName)
...
-------------------------------------------------------------------------------------------------
function IGERefreshReligions()
	g_ReligionButtonManager:ResetInstances();
	local pActivePlayer = g_player;
	...
	for row in GameInfo.Religions("Type <> 'RELIGION_PANTHEON'") do
		...
		religionButton.Button:SetToolTipString(Locale.Lookup(row.Description));
		religionButton.Icon:SetTexture("Assets/DLC/Expansion/UI/Art/Icons/ReligiousSymbolsWhite64_Expansion.dds");
		IconHookup(row.PortraitIndex, 64, row.IconAtlas, religionButton.Icon);
		
		religionButton.Button:SetDisabled(false);
			
		if(g_CurrentReligionID == row.ID) then
			religionButton.Button:SetDisabled(true);
		elseif(takenReligions[row.ID] ~= nil) then
			religionButton.Button:SetDisabled(true);
		
			local t = Locale.Lookup("TXT_KEY_CHOOSE_RELIGION_ALREADY_FOUNDED", row.Description, takenReligions[row.ID]);
			religionButton.Button:SetToolTipString(t);
		end
		
		if (g_bFoundingReligion) then
			religionButton.Button:RegisterCallback(Mouse.eLClick, function() IGESelectReligion(row.ID, row.Description, row.IconAtlas, row.PortraitIndex); end);
		else
			religionButton.Button:RegisterCallback(Mouse.eLClick, IGEIgnoreLeftClick);		
		end
		
		numReligionButtons = numReligionButtons + 1;
	end	
		
	if(numReligionButtons > 13) then
...
-------------------------------------------------------------------------------------------------
function IGERefreshExistingBeliefs()
	...
	if (pPlayer:HasCreatedReligion()) then
		...
		Controls.PantheonBeliefButton:RegisterCallback(Mouse.eLClick, IGEIgnoreLeftClick);
		Controls.AlphaAnimPantheon:SetHide(true);
		Controls.FollowerBeliefButton:RegisterCallback(Mouse.eLClick, IGEIgnoreLeftClick);
		Controls.AlphaAnimFollower:SetHide(true);
		Controls.FounderBeliefButton:RegisterCallback(Mouse.eLClick, IGEIgnoreLeftClick);
        Controls.AlphaAnimFounder:SetHide(true);
		Controls.FollowerBelief2Button:RegisterCallback(Mouse.eLClick, IGEOnFollowerBelief2Click);
		...
		Controls.EnhancerBeliefButton:RegisterCallback(Mouse.eLClick, IGEOnEnhancerBeliefClick);
	...
	elseif (pPlayer:HasCreatedPantheon()) then
		...
		Controls.PantheonBeliefButton:RegisterCallback(Mouse.eLClick, IGEIgnoreLeftClick);
		Controls.AlphaAnimPantheon:SetHide(true);
		Controls.FollowerBeliefButton:RegisterCallback(Mouse.eLClick, IGEOnFollowerBeliefClick);
		Controls.AlphaAnimFollower:SetHide(false);
		Controls.FounderBeliefButton:RegisterCallback(Mouse.eLClick, IGEOnFounderBeliefClick);
        Controls.AlphaAnimFounder:SetHide(false);
		Controls.FollowerBelief2Button:RegisterCallback(Mouse.eLClick, IGEIgnoreLeftClick);
		...
		Controls.EnhancerBeliefButton:RegisterCallback(Mouse.eLClick, IGEIgnoreLeftClick);
    ...
    -- Not even a pantheon
		...
		Controls.PantheonBeliefButton:RegisterCallback(Mouse.eLClick, IGEOnPantheonBeliefClick);
	...
	if (pPlayer:IsTraitBonusReligiousBelief()) then
		if (not pPlayer:HasCreatedReligion()) then
			Controls.BonusBeliefButton:RegisterCallback(Mouse.eLClick, IGEOnBonusBeliefClick);
 			Controls.BonusBeliefDescription:LocalizeAndSetText("TXT_KEY_CHOOSE_RELIGION_SELECT_BONUS_BELIEF");
			Controls.BonusBeliefDescription:SetOffsetVal(24,25);
            Controls.AlphaAnimBonus:SetHide(false);
			Controls.BonusLock:SetHide(true);
		else
 			Controls.BonusBeliefButton:RegisterCallback(Mouse.eLClick, IGEIgnoreLeftClick);
			Controls.BonusBeliefDescription:SetOffsetVal(24,25);
...
-------------------------------------------------------------------------------------------------
function IGESelectReligion(religionID, name, iconAtlas, portraitIndex)
	...
	if (g_CurrentReligionID ~= nil) then		
		...
		if (pPlayer:HasCreatedReligion()) then
			...
			Controls.LabelReligionName:RegisterCallback(Mouse.eLClick, IGEIgnoreLeftClick);
			...
			Controls.LabelReligionName:RegisterCallback(Mouse.eLClick, IGEChangeReligionName);
...
-------------------------------------------------------------------------------------------------
function IGEChangeReligionName()
...
-------------------------------------------------------------------------------------------------
function IGESelectFromBeliefs(beliefs, selectFn)
...
function IGEOnYes( )
	...
	if g_bFoundingReligion then
		...
		Game.FoundReligion(g_player:GetID(), g_CurrentReligionID, customName, beliefsToSend[1], beliefsToSend[2], beliefsToSend[3], beliefsToSend[4], g_city);
	else
		Game.EnhanceReligion(g_player:GetID(), g_CurrentReligionID, g_Beliefs[4], g_Beliefs[5]);
	end
...
Controls.ChangeNameOKButton:RegisterCallback(Mouse.eLClick, IGEOnChangeNameOK);

-------------------------------------------------------------------------------------------------
function IGEOnChangeNameDefault()
	local defaultName = Locale.Lookup(GameInfo.Religions[g_CurrentReligionID].Description);
	Controls.NewName:SetText(defaultName);
	Controls.ChangeNameError:SetHide(true);
end
Controls.ChangeNameDefaultButton:RegisterCallback(Mouse.eLClick, IGEOnChangeNameDefault);

-------------------------------------------------------------------------------------------------
function IGEOnChangeNameCancel()
	Controls.ChangeNamePopup:SetHide(true);
end
Controls.ChangeNameCancelButton:RegisterCallback(Mouse.eLClick, IGEOnChangeNameCancel);

-------------------------------------------------------------------------------------------------
function IGEFoundReligion()
    local t;
    if (g_bFoundingReligion) then
...
Controls.FoundReligion:RegisterCallback(Mouse.eLClick, IGEFoundReligion);

-------------------------------------------------------------------------------------------------
function IGEShowHideHandler( bIsHide, bInitState )
    if( not bInitState ) then
        if( not bIsHide ) then
			Controls.TopDecoratorCenter:SetTexture("assets/DLC/Expansion/UI/Art/Icons/Religion128.dds");
			Controls.DecoratorBackground:SetTexture("Found_Religion_background.dds");
			Controls.ReligionIcon:SetTexture("Assets/DLC/Expansion/UI/Art/Icons/ReligiousSymbolsWhite80_Expansion.dds");

        	IGERefreshReligions();
        	IGERefreshExistingBeliefs();
        	    	
			if (g_CurrentReligionID ~= nil) then
				local religionRow = GameInfo.Religions[g_CurrentReligionID];
				IGESelectReligion(g_CurrentReligionID, Game.GetReligionName(g_CurrentReligionID), religionRow.IconAtlas, religionRow.PortraitIndex);
			else
...
LuaEvents.IGE_SelectedPlayer.Add(IGEOnActivePlayerChanged);
