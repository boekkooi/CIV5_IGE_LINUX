...
-------------------------------------------------------------------------------------------------
local startingPlots = {};
local function IGEUpdatePlotCore(plot, instance, riverInstance, cityInstance, fogInstance)
	-- Terrain or fog
...
function IGEUpdateCore()
	...
	for i = 0, Map.GetNumPlots()-1, 1 do
		IGEUpdatePlotCore(Map.GetPlotByIndex(i), hexInstances[i], riverInstances[i], cityInstances[i], fogInstances[i])
	end
end
Events.GameplaySetActivePlayer.Add(IGEUpdateCore);

-------------------------------------------------------------------------------------------------
function IGEInitialize()
	for i = 0, Map.GetNumPlots()-1, 1 do
...
-------------------------------------------------------------------------------------------------
function IGEUpdatePlot(plot)
	if ContextPtr:IsHidden() or (not initialized) then return end
	local index = GetPlotIndex(plot)
	IGEUpdatePlotCore(plot, hexInstances[index], riverInstances[index], cityInstances[index], fogInstances[index])
end
LuaEvents.IGE_ModifiedPlot.Add(IGEUpdatePlot);
...
local function OnClearHighlights(abc)
	for index in pairs(highlights) do
		...
		IGEUpdatePlot(Map.GetPlotByIndex(index))
	end
	highlights = {}
end
...
local function OnOptionsChanged()
	if not CheckVisibility() then IGEUpdateCore() end
end
