...
-- global constants
------------------------------------------------
g_strInfiniteTurns = IGEL("TXT_KEY_PRODUCTION_HELP_INFINITE_TURNS");
...
-------------------------------------------------
function IGEProductionSelected( eOrder, iData)
...
-------------------------------------------------
-------------------------------------------------
function IGEOnCityButton( x, y )
    IGEOnClose();
    UI.DoSelectCityAtPlot( Map.GetPlot( x, y ) );
end
Controls.CityButton:RegisterCallback( Mouse.eLClick, IGEOnCityButton );
...
function IGEOnBuildingHeaderSelected()
	buildingHeadingOpen = not buildingHeadingOpen;
	IGEUpdateWindow( g_currentCity );
end

function IGEOnUnitHeaderSelected()
	unitHeadingOpen = not unitHeadingOpen;
	IGEUpdateWindow( g_currentCity );
end

function IGEOnWonderHeaderSelected()
	wonderHeadingOpen = not wonderHeadingOpen;
	IGEUpdateWindow( g_currentCity );
end

function IGEOnOtherHeaderSelected()
	otherHeadingOpen = not otherHeadingOpen;
	IGEUpdateWindow( g_currentCity );
end


function IGEUpdateWindow( city )
	...
	if (city:IsFoodProduction() or city:FoodDifferenceTimes100() == 0) then
		Controls.CityGrowthLabel:SetText(IGEL("TXT_KEY_CITYVIEW_STAGNATION_TEXT"));
	elseif city:FoodDifference() < 0 then
		Controls.CityGrowthLabel:SetText(IGEL("TXT_KEY_CITYVIEW_STARVATION_TEXT"));
	else
		Controls.CityGrowthLabel:SetText(IGEL("TXT_KEY_CITYVIEW_TURNS_TILL_CITIZEN_TEXT", cityGrowth));
	end
 		...
 		if g_IsProductionMode then
			...
			if city:CanTrain( unitID, 0, 1 ) then
				...
				if (bGeneratingProduction) then
					strTurnsLeft = IGEL( "TXT_KEY_STR_TURNS", city:GetUnitProductionTurnsLeft( unitID ));
				else
					strTurnsLeft = g_strInfiniteTurns;
				end
				
				IGEAddProductionButton( unitID, unit.Description, OrderTypes.ORDER_TRAIN, strTurnsLeft, 1, isDisabled );
			...
			if city:IsCanPurchase(true, unitID, -1, -1) then
				...
				IGEAddProductionButton( unitID, unit.Description, OrderTypes.ORDER_TRAIN, cost, 1, isDisabled);
				numUnitButtons = numUnitButtons + 1;
			end
 		end
	end
	if unitHeadingOpen then
		local localizedLabel = "[ICON_MINUS] "..IGEL( "TXT_KEY_POP_UNITS" );
		Controls.UnitButtonLabel:SetText(localizedLabel);
		Controls.UnitButtonStack:SetHide( false );
	else
		local localizedLabel = "[ICON_PLUS] "..IGEL( "TXT_KEY_POP_UNITS" );
		Controls.UnitButtonLabel:SetText(localizedLabel);
	...
	Controls.UnitButton:RegisterCallback( Mouse.eLClick, IGEOnUnitHeaderSelected );
 		...
 		local projectID = project.ID;
 	 	if g_IsProductionMode then
			...
			if city:CanCreate( projectID, 0, 1 ) then
				...
				if (bGeneratingProduction) then
					strTurnsLeft = IGEL( "TXT_KEY_STR_TURNS", city:GetProjectProductionTurnsLeft( projectID ));
				else
					strTurnsLeft = g_strInfiniteTurns;
				end
				
				IGEAddProductionButton( projectID, project.Description, OrderTypes.ORDER_CREATE, strTurnsLeft, 3, isDisabled );
			...
			if city:IsCanPurchase(true, -1, -1, projectID) then
				...
				IGEAddProductionButton( projectID, project.Description, OrderTypes.ORDER_CREATE, cost, 3, isDisabled );
				numWonderButtons = numWonderButtons + 1;
			end
		end
	end
 	...
 	for building in GameInfo.Buildings() do
 		...
 		if g_IsProductionMode then
			...
			if city:CanConstruct( buildingID, 0, 1 ) then
				...
				if (bGeneratingProduction) then
					strTurnsLeft = IGEL( "TXT_KEY_STR_TURNS", city:GetBuildingProductionTurnsLeft( buildingID ));
				else
					strTurnsLeft = g_strInfiniteTurns;
				end
				
				IGEAddProductionButton( buildingID, building.Description, OrderTypes.ORDER_CONSTRUCT, strTurnsLeft, col, isDisabled );
				if col == 2 then
			...
			if city:IsCanPurchase(true, -1, buildingID, -1) then
				...
				IGEAddProductionButton( buildingID, building.Description, OrderTypes.ORDER_CONSTRUCT, cost, col, isDisabled );
				if col == 2 then
					numBuildingButtons = numBuildingButtons + 1;
				else
					numWonderButtons = numWonderButtons + 1;
	...
	if buildingHeadingOpen then
		local localizedLabel = "[ICON_MINUS] "..IGEL( "TXT_KEY_POP_BUILDINGS" );
		Controls.BuildingsButtonLabel:SetText(localizedLabel);
		Controls.BuildingButtonStack:SetHide( false );
	else
		local localizedLabel = "[ICON_PLUS] "..IGEL( "TXT_KEY_POP_BUILDINGS" );
		Controls.BuildingsButtonLabel:SetText(localizedLabel);
	...
	Controls.BuildingsButton:RegisterCallback( Mouse.eLClick, IGEOnBuildingHeaderSelected );
	
	if wonderHeadingOpen then
		local localizedLabel = "[ICON_MINUS] "..IGEL( "TXT_KEY_POP_WONDERS" );
		Controls.WondersButtonLabel:SetText(localizedLabel);
		Controls.WonderButtonStack:SetHide( false );
	else
		local localizedLabel = "[ICON_PLUS] "..IGEL( "TXT_KEY_POP_WONDERS" );
		Controls.WondersButtonLabel:SetText(localizedLabel);
	...
	Controls.WondersButton:RegisterCallback( Mouse.eLClick, IGEOnWonderHeaderSelected );
 		...
 		for process in GameInfo.Processes() do
			...
			if city:CanMaintain( processID, 1 ) then
 				if process.Type == "PROCESS_WEALTH" then
					...
					Controls.ProduceGoldButton:RegisterCallback( Mouse.eLClick, IGEProductionSelected );
				...
				elseif process.Type == "PROCESS_RESEARCH" then
					...
					Controls.ProduceResearchButton:RegisterCallback( Mouse.eLClick, IGEProductionSelected );
					numOtherButtons = numOtherButtons + 1;
				end
			end
		end
	...
	if otherHeadingOpen then
		local localizedLabel = "[ICON_MINUS] "..IGEL( "TXT_KEY_CITYVIEW_OTHER" );
		Controls.OtherButtonLabel:SetText(localizedLabel);
	else
		local localizedLabel = "[ICON_PLUS] "..IGEL( "TXT_KEY_CITYVIEW_OTHER" );
		Controls.OtherButtonLabel:SetText(localizedLabel);
		Controls.ProduceGoldButton:SetHide( true );
		Controls.ProduceResearchButton:SetHide( true );
	end
	...
	Controls.OtherButton:RegisterCallback( Mouse.eLClick, IGEOnOtherHeaderSelected );
	...
	if (m_gOrderType == -1 or m_gOrderType > 4) then
		...
		strItemName = IGEL(city:GetProductionNameKey());
	...
	if unitProduction ~= -1 then
		...
		if IconHookup( thisUnitInfo.PortraitIndex, 128, thisUnitInfo.IconAtlas, Controls.ProductionPortrait ) then
			...
			-- Info for this thing
			strToolTip = IGEL(GetHelpTextForUnit(unitProduction, true)) .. "[NEWLINE][NEWLINE]" .. strToolTip;
	...
	elseif buildingProduction ~= -1 then
		...
		if IconHookup( thisBuildingInfo.PortraitIndex, 128, thisBuildingInfo.IconAtlas, Controls.ProductionPortrait ) then
			...
			-- Info for this thing
			strToolTip = IGEL(GetHelpTextForBuilding(buildingProduction, false, false, false)) .. "[NEWLINE][NEWLINE]" .. strToolTip;
	...
	elseif projectProduction ~= -1 then
		...
		if IconHookup( thisProjectInfo.PortraitIndex, 128, thisProjectInfo.IconAtlas, Controls.ProductionPortrait ) then
			...
			-- Info for this thing
			strToolTip = IGEL(GetHelpTextForProject(projectProduction, true)) .. "[NEWLINE][NEWLINE]" .. strToolTip;
	...
	if (not bJustFinishedSomething) then
		...
		local strProductionPerTurn = IGEL("TXT_KEY_CITY_SCREEN_PROD_PER_TURN", iProductionPerTurn);
		...
		if productionTurnsLeft > 99 then
			strNumTurns = IGEL("TXT_KEY_PRODUCTION_HELP_99PLUS_TURNS");
		else
			strNumTurns = IGEL("TXT_KEY_PRODUCTION_HELP_NUM_TURNS", productionTurnsLeft);
		end
	...
	if qMode then
		...
		for i = 1, qLength do
			...
			if (queuedOrderType == OrderTypes.ORDER_TRAIN) then
				...
				Controls[controlName]:SetText( IGEL( thisUnitInfo.Description ) );
				Controls[controlTurns]:SetText( IGEL("TXT_KEY_PRODUCTION_HELP_NUM_TURNS", city:GetUnitProductionTurnsLeft(queuedData1, i-1) ) );
			...
			elseif (queuedOrderType == OrderTypes.ORDER_CONSTRUCT) then
				...
				Controls[controlName]:SetText( IGEL( thisBuildingInfo.Description ) );
				Controls[controlTurns]:SetText( IGEL("TXT_KEY_PRODUCTION_HELP_NUM_TURNS", city:GetBuildingProductionTurnsLeft(queuedData1, i-1)) );
			...
			elseif (queuedOrderType == OrderTypes.ORDER_CREATE) then
				...
				Controls[controlName]:SetText( IGEL( thisProjectInfo.Description ) );
				Controls[controlTurns]:SetText( IGEL("TXT_KEY_PRODUCTION_HELP_NUM_TURNS", city:GetProjectProductionTurnsLeft(queuedData1, i-1)) );
			...
			elseif (queuedOrderType == OrderTypes.ORDER_MAINTAIN) then
				...
				Controls[controlName]:SetText( IGEL( thisProcessInfo.Description ) );
				Controls[controlTurns]:SetHide( true );
				if (thisProcessInfo.Help ~= nil) then
					strToolTip = thisProcessInfo.Help;
				end
			end
			Controls[controlBox]:SetToolTipString(IGEL(strToolTip));
...
function IGEOnCityDestroyed(hexPos, playerID, cityID, newPlayerID)
	if not bHidden then
		local city = g_currentCity;
...
function IGEOnPortraitRClicked()
	...
	if unitProduction ~= -1 then
		local thisUnitInfo = GameInfo.Units[unitProduction];
		searchString = IGEL( thisUnitInfo.Description );
	elseif buildingProduction ~= -1 then
		local thisBuildingInfo = GameInfo.Buildings[buildingProduction];
		searchString = IGEL( thisBuildingInfo.Description );
	elseif projectProduction ~= -1 then
		local thisProjectInfo = GameInfo.Projects[projectProduction];
		searchString = IGEL( thisProjectInfo.Description );
	elseif processProduction ~= -1 then
		local pProcessInfo = GameInfo.Processes[processProduction];
		searchString = IGEL( pProcessInfo.Description );
	else
		noProduction = true;
	end
...
Controls.ProductionPortraitButton:RegisterCallback( Mouse.eRClick, IGEOnPortraitRClicked );
...
----------------------------------------------------------------        
function IGEAddProductionButton( id, description, orderType, turnsLeft, column, isDisabled )	
	local controlTable;
	...
	if column == 1 then -- we are a unit
		...
		local strToolTip = IGEL(GetHelpTextForUnit(id, bIncludeRequirementsInfo));
    ...
    local nameString = IGEL( description );
    ...
    controlTable.Button:RegisterCallback( Mouse.eLClick, IGEProductionSelected );
    controlTable.Button:RegisterCallback( Mouse.eRClick, IGEGetProdHelp );
...
Controls.CloseButton:RegisterCallback( Mouse.eLClick, IGEOnClose );
...
-------------------------------------------------------------------------------
-------------------------------------------------------------------------------
function IGEShowHideHandler( bIsHide, bInitState )
...
ContextPtr:SetShowHideHandler(IGEShowHideHandler);


-------------------------------------------------------------------------------
-------------------------------------------------------------------------------
function IGEOnPurchaseButton()
	g_IsProductionMode = not g_IsProductionMode;
	IGEUpdateWindow( g_currentCity );
end
Controls.PurchaseButton:RegisterCallback( Mouse.eLClick, IGEOnPurchaseButton );

----------------------------------------------------------------
-- 'Active' (local human) player has changed
----------------------------------------------------------------
function IGEOnActivePlayerChanged()
...
local function IGEOnShowing()
	Events.SerialEventGameMessagePopup.Add(IGEOnPopup);

	Events.SerialEventCityDestroyed.Add(IGEOnCityDestroyed);
	Events.SerialEventCityCaptured.Add(IGEOnCityDestroyed);

	Events.SerialEventExitCityScreen.Add(IGEOnClose);
	Events.GameplaySetActivePlayer.Add(IGEOnActivePlayerChanged);

	Events.SerialEventCityScreenDirty.Add(IGEOnDirty);
	Events.SerialEventCityInfoDirty.Add(IGEOnDirty);
	Events.SerialEventGameDataDirty.Add(IGEOnDirty);
	Events.SerialEventUnitInfoDirty.Add(IGEOnDirty);
	Events.UnitSelectionChanged.Add(IGEOnDirty);
	Events.UnitGarrison.Add(IGEOnDirty);
	Events.UnitEmbark.Add(IGEOnDirty);
end
...
local function IGEOnClosing()
	...
	Events.SerialEventGameMessagePopup.Remove(IGEOnPopup);

	Events.SerialEventCityDestroyed.Remove(IGEOnCityDestroyed);
	Events.SerialEventCityCaptured.Remove(IGEOnCityDestroyed);

	Events.SerialEventExitCityScreen.Remove(IGEOnClose);
	Events.GameplaySetActivePlayer.Remove(IGEOnActivePlayerChanged);

	Events.SerialEventCityScreenDirty.Remove(IGEOnDirty);
	Events.SerialEventCityInfoDirty.Remove(IGEOnDirty);
	Events.SerialEventGameDataDirty.Remove(IGEOnDirty);
	Events.SerialEventUnitInfoDirty.Remove(IGEOnDirty);
	Events.UnitSelectionChanged.Remove(IGEOnDirty);
	Events.UnitGarrison.Remove(IGEOnDirty);
	Events.UnitEmbark.Remove(IGEOnDirty);
end
