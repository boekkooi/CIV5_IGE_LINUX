...
local function GetResourceString(plot)
	...
	local convertedKey = IGEL(pResource.Description);		
	...
	if iTechCityTrade ~= nil and iTechCityTrade ~= -1 then
		...
		if not pTeam:GetTeamTechs():HasTech(iTechCityTrade) then
			...
			improvementStr = improvementStr .. " " .. IGEL( "TXT_KEY_PLOTROLL_REQUIRES_TECH_TO_USE", techName );
		end
	end
...
local function GetToolTipText(plot)
	...
	for pBuildInfo in GameInfo.Builds() do
		...
		if iTurnsLeft < 4000 and iTurnsLeft > 0 then
			...
			local convertedKey = IGEL(pBuildInfo.Description);
			str = str..IGEL("TXT_KEY_WORKER_BUILD_PROGRESS", iTurnsLeft, convertedKey);
		end
	end
	...
	if strResource ~= "" then
		if str ~= "" then str = str.."[NEWLINE]" end
		str = str.."[COLOR_POSITIVE_TEXT]" .. IGEL("TXT_KEY_RESOURCE") .. "[ENDCOLOR]" .. " : " .. strResource;
	end	
	...
	if strImprovement ~= "" or plot:IsTradeRoute() then
		...
		if strImprovement ~= "" then
			str = str.."[COLOR_POSITIVE_TEXT]" .. IGEL("TXT_KEY_IMPROVEMENT") .. "[ENDCOLOR]" .. " : " .. strImprovement;
			strTradeRouteBlock = ", ";
		end
		
		-- Trade Route
		if plot:IsTradeRoute() then
			str = str..strTradeRouteBlock..IGEL( "TXT_KEY_PLOTROLL_TRADE_ROUTE" );
		end
	end	
	...
	if natureStr ~= "" then
		if str ~= "" then str = str.."[NEWLINE]" end
		str = str.."[COLOR_POSITIVE_TEXT]" .. IGEL("TXT_KEY_PEDIA_TERRAIN_LABEL") .. "[ENDCOLOR]" .. " : " .. natureStr;
	end
	...
	if strYield ~= "" then
		if str ~= "" then str = str.."[NEWLINE]" end
		str = str.."[COLOR_POSITIVE_TEXT]" .. IGEL("TXT_KEY_OUTPUT") .. "[ENDCOLOR]" .. " : " .. strYield;
	end
		
	-- Presence of fresh water
	if plot:IsFreshWater() then
		if str ~= "" then str = str.."[NEWLINE]" end
		str = str..IGEL("TXT_KEY_PLOTROLL_FRESH_WATER");
	end
...
-------------------------------------------------------------------------------------------------
local function IGEOnBroadcastingMouseState(mouseOver, gridX, gridY, plot)
	if mouseOver then
...
-------------------------------------------------------------------------------------------------
local function IGEOnModifiedPlot(plot)
	if initialized then
