...
-------------------------------------------------------------------------------------------------
function IGEGetSideIndex(side)
	if type(side) == "number" then return side % 6 end
	return sidesToIndices[side];
end

-------------------------------------------------------------------------------------------------
function IGEGetVertexIndex(vertex)
	if type(vertex) == "number" then return vertex % 6 end
	return verticesToIndices[vertex];
end

-------------------------------------------------------------------------------------------------
function IGEGetAdjacentPlot(plot, side)
	side = IGEGetSideIndex(side);
	return Map.PlotDirection(plot:GetX(), plot:GetY(), sidesToDirections[side]);
end

-------------------------------------------------------------------------------------------------
function IGEGetFlowRotation(plot, side)
	side = IGEGetSideIndex(side);

	-- Need to check adjacent side
	if side > 2 then
		local adjacentPlot = IGEGetAdjacentPlot(plot, side);
		if adjacentPlot then
			local rotation = IGEGetFlowRotation(adjacentPlot, (side + 3) % 6); 
...
-------------------------------------------------------------------------------------------------
function IGESetFlowRotation(plot, side, rotation)
	if plot then
		side = IGEGetSideIndex(side);
		--print(side);
		...
		if side > 2 then
			...
			if rotation == NoRotation then
				IGESetFlowRotation(adjacentPlot, (side + 3) % 6, NoRotation);
			else
				IGESetFlowRotation(adjacentPlot, (side + 3) % 6, -rotation);
			end
		else
...
local function GetConstrainedRotation(plot, side)
	...
	if plot then
		...
		for i, plotIndex in ipairs(adjacentPlots) do
			...
			if plotIndex >= 0 then
				adjacentPlot = IGEGetAdjacentPlot(plot, plotIndex);
			end

			if adjacentPlot then
				local rotation = IGEGetFlowRotation(adjacentPlot, adjacentSides[i]);
				if rotation ~= NoRotation then
...
-------------------------------------------------------------------------------------------------
function IGEToggleRiverFlow(plot, side)
	if plot then 
		side = IGEGetSideIndex(side);
		local current = IGEGetFlowRotation(plot, side);
		...
		if constrain ~= NoRotation then
			if current ~= NoRotation then
				IGESetFlowRotation(plot, side, NoRotation);
			else
				IGESetFlowRotation(plot, side, constrain);
			end
		-- Beginning of a river: we cycle between "flow1", "flow2" and "off".
		else
			if current == CWRotation then
				IGESetFlowRotation(plot, side, CCWRotation);
			elseif current == CCWRotation then
				IGESetFlowRotation(plot, side, NoRotation);
			else
				IGESetFlowRotation(plot, side, CWRotation);
			end
		end
	end
end
...
-------------------------------------------------------------------------------------------------
function IGEIsRiverEntryPoint(plot, vertex)
	vertex = IGEGetVertexIndex(vertex);
	...
	local adjacent = IGEGetAdjacentPlot(plot, vertex);
	if not adjacent then
		adjacent = IGEGetAdjacentPlot(plot, (vertex - 1) % 6);
		adjacentSide = (vertex - 5) % 6;
	end
	if adjacent and IGEGetFlowRotation(adjacent, adjacentSide) ~= NoRotation then 
		return true;
	end
	...
	if IGEGetFlowRotation(plot, vertex) ~= NoRotation then
		adjacent = IGEGetAdjacentPlot(plot, (vertex - 1) % 6);
		if adjacent and adjacent:GetPlotType() == PlotTypes.PLOT_OCEAN  then
			return true;
		end
	end
	if IGEGetFlowRotation(plot, (vertex - 1) % 6) ~= NoRotation then
		adjacent = IGEGetAdjacentPlot(plot, vertex);
		if adjacent and adjacent:GetPlotType() == PlotTypes.PLOT_OCEAN  then
			return true;
		end
	end
