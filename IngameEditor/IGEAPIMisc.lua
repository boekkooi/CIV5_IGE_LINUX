...
function IGEGetYieldIcon(yieldType)
...
function IGEGetYieldString(item)
	...
	if str == "" and item.improvedResources and next(item.improvedResources) then
		...
		for _, row in pairs(lookup) do
			...
			local yieldIcon = IGEGetYieldIcon(yieldType)
...
local yieldChangeTemplate = IGEL("TXT_KEY_IGE_YIELD_CHANGE_TEMPLATE");
local noYieldChangeTemplate = IGEL("TXT_KEY_IGE_NO_YIELD_CHANGE_TEMPLATE");

function IGEGetYieldChangeString(item)
	local str = "";
	...
	if item.yieldChanges then
		for i, v in ipairs(item.yieldChanges) do
			local suffix = nil;
			local value = nil;
			suffix = IGEGetYieldIcon(v.yieldType)
			...
			if suffix then
				...
				str = str..IGEL("TXT_KEY_IGE_YIELD_CHANGE_TEMPLATE", amount, sourceName);
			else
				str = str..IGEL("TXT_KEY_IGE_NO_YIELD_CHANGE_TEMPLATE", sourceName);
			end
		end
	end
...
-- OTHERS
--===============================================================================================
function IGEGetXPForLevel(level)
...
-------------------------------------------------------------------------------------------------
function IGEGetSelection(items)
...
-------------------------------------------------------------------------------------------------
function IGEGetTeamID(playerID)
	local pPlayer = Players[playerID];
	return pPlayer:GetTeam();
end

-------------------------------------------------------------------------------------------------
function IGEGetTeam(playerID)
	local pPlayer = Players[playerID];
...
-------------------------------------------------------------------------------------------------
function IGEDeleteFiles(fileName)
...
-------------------------------------------------------------------------------------------------
function IGESaveFile(canCleanUpFile)
	...
	LuaEvents.IGE_FloatingMessage(IGEL("TXT_KEY_IGE_SAVED_AS", fileName));
	return fileName;
end